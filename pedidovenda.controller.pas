...
type
  iPedidoVendaController = interface
    ...
    procedure editaProdGrid(pEditIndex, pEditCodProd, pEditDescrProd, pEditQtdProd, pEditVlrUnitProd, pEditVlrTotalPed: TEdit; pLsvGrid: TListView);
...
begin
  try
    ...
    begin
      ...
      begin
        if Trim(TEdit(pEditQtdProd).Text) = EmptyStr then
        begin
...
var
  ...
  iItem: TListItem;
begin
  if Trim(pCodProd) = EmptyStr then
  begin
    if pEditCodProd <> nil then
    begin
...
procedure TPedidoVendaController.editaProdGrid(pEditIndex, pEditCodProd, pEditDescrProd, pEditQtdProd, pEditVlrUnitProd, pEditVlrTotalPed: TEdit; pLsvGrid: TListView);
var
  iItem: TListItem;
begin
  ...
  begin
    ...
    if pEditQtdProd <> nil then
    begin
      if TEdit(pEditQtdProd).CanFocus then
      begin
        TEdit(pEditQtdProd).SetFocus;
      end;
    end;
  end;
end;
...
begin
  if pLsvGrid.SelCount > 0 then
  begin
    if Application.MessageBox(Pchar(Format('Deseja excluir ítem %s - %s do grid ?', [pLsvGrid.Selected.SubItems[0], pLsvGrid.Selected.SubItems[1]])), Pchar('Exclusão ítem'), mb_yesNo + mb_IconQuestion) = mrYes then
    begin
    ...
    begin
      ...
      try
        ...
        begin
          ...
          iItem.SubItems.Add(FormatFloat('0.0000', Query.FieldByName('quantidade').AsFloat));
      ...
      if pFormPed.lsvItens.CanFocus then
      begin
        if pFormPed.lsvItens.Items.Count > 0 then
        begin
          pFormPed.lsvItens.Items[0].Selected := True;
          pFormPed.lsvItens.SetFocus;
        end;
      end;
    end
    else
