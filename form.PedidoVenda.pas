...
type
  TformPedidoVenda = class(TForm)
    ...
    btnAdicionarProdutoPedido: TButton;
...
procedure TformPedidoVenda.btnAdicionarProdutoPedidoClick(Sender: TObject);
begin
  TPedidoVendaController
    .New(form.Principal.FConexao)
    .adicionaProdGrid( edtCodProd.Text, edtItemIndex, edtCodProd, edtDescrProd, edtQtdProd, edtVlrUnitProd, edtVlrTotalPed, lsvItens );
end;
...
begin
  TPedidoVendaController
    .New(form.Principal.FConexao)
    .carregarCliente( StrToIntDef(TEdit(Sender).Text, 0), TEdit(Sender), edtNomeCliente );
end;

procedure TformPedidoVenda.edtCodClienteKeyPress(Sender: TObject;  var Key: Char);
begin
  EnterKeyPress(Sender, Key, Self);
end;
...
begin
  TPedidoVendaController
    .New(form.Principal.FConexao)
    .carregarProduto( TEdit(Sender).Text, TEdit(Sender), edtDescrProd, edtQtdProd, edtVlrUnitProd );
end;

procedure TformPedidoVenda.edtCodProdKeyPress(Sender: TObject; var Key: Char);
begin
  EnterKeyPress(Sender, Key, Self);
end;
...
begin
  utilitarios.FormatarDecimaisExit(TEdit(Sender), 4);
end;

procedure TformPedidoVenda.edtQtdProdKeyPress(Sender: TObject; var Key: Char);
begin
  utilitarios.ValorKeyPress(Sender, Key, 4);
  EnterKeyPress(Sender, Key, Self);
end;
...
begin
  TPedidoVendaController
    .New(form.Principal.FConexao)
    .editaProdGrid( edtItemIndex, edtCodProd, edtDescrProd, edtQtdProd, edtVlrUnitProd, edtVlrTotalPed, lsvItens );
end;
...
begin
  case Key of
    ...
    begin
      TPedidoVendaController
        .New(form.Principal.FConexao)
        .removerProdGrid( edtVlrTotalPed, lsvItens );
    end;
  end;
end;
